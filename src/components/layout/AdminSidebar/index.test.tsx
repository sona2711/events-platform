...
describe('AdminSidebar', () => {
  ...
  it('renders footer actions and admin profile', () => {
    ...
    expect(screen.getByRole('button', { name: commonEn.adminNav.createEvent })).toBeInTheDocument()
    expect(screen.getByText(commonEn.adminSidebar.adminName)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: commonEn.adminNav.logout })).toBeInTheDocument()
  })
  ...
  it('opens and closes the mobile navigation menu', async () => {
    ...
    await user.click(screen.getByRole('button', { name: commonEn.adminSidebar.toggleMenu }))

    expect(
      screen.getByRole('button', { name: commonEn.adminSidebar.closeMenu }),
    ).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: commonEn.adminSidebar.closeMenu }))
